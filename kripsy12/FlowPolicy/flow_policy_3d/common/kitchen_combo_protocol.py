...
def aggregate_seed_metrics(
    ...
    all_episodes_pooled: List[Dict[str, Any]] = []
    per_combo_pk: Dict[str, List[float]] = {f"p{k}": [] for k in range(1, NUM_SUBTASKS + 1)}
    per_task_success_pooled: Dict[str, List[float]] = {t: [] for t in ALL_TASKS}
    ...
    for cm in combo_metrics:
        ...
        for task in cm["tasks"]:
            ...
            if sr["mean"] is not None:
                per_task_success_pooled[task].extend([sr["mean"]] * sr["n_samples"])
    ...
    # Per-task success across all episodes where task appears
    per_task_appearances: Dict[str, List[float]] = {t: [] for t in ALL_TASKS}
    for cm in combo_metrics:
        for task in cm["tasks"]:
            sr = cm["per_task_success"][task]
            if sr["n_samples"] > 0:
                # Reconstruct from per-episode if available in episodes field
                pass
