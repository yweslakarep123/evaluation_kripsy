...
@click.option(
    ...
    default=None,
    type=int,
    help="Override policy.num_inference_step (NFE). Default: checkpoint config.",
)
@click.option(
    "--sampling_seed",
    default=None,
...
def main(
    ...
    for ckpt_path in ckpt_paths:
        ...
        effective_nfe = getattr(policy, "num_inference_step", None)
        if num_inference_steps is not None:
            if not hasattr(policy, "num_inference_step"):
                raise click.ClickException(
                    "Policy has no num_inference_step attribute to override"
                )
            policy.num_inference_step = num_inference_steps
