if __name__ == "__main__":
    ...
    # Direktori yang memuat train.py dan paket flow_policy_3d (bukan tiga tingkat ke atas).
    _train_root = pathlib.Path(__file__).resolve().parent
    sys.path.insert(0, str(_train_root))
    os.chdir(str(_train_root))
...
def _agent_debug_cuda_snapshot(location: str, hypothesis_id: str, extra=None):
    """NDJSON debug line for CUDA OOM investigation (no secrets)."""
    data = dict(extra or {})
    try:
        if torch.cuda.is_available():
            try:
                free_b, total_b = torch.cuda.mem_get_info()
                data["cuda_free_mb"] = round(free_b / (1024**2), 2)
                data["cuda_total_mb"] = round(total_b / (1024**2), 2)
                data["torch_alloc_mb"] = round(
                    torch.cuda.memory_allocated() / (1024**2), 2
                )
                data["torch_reserved_mb"] = round(
                    torch.cuda.memory_reserved() / (1024**2), 2
                )
            except Exception as e:
                data["cuda_mem_query_error"] = str(e)[:240]
        try:
            ...
            data["nvidia_smi_used_free_mib"] = smi[:200]
        except Exception as e:
            data["nvidia_smi_error"] = str(e)[:120]
        payload = {
            "sessionId": _AGENT_DEBUG_SESSION,
            "location": location,
            "message": "cuda_snapshot",
            "data": data,
            "timestamp": int(time.time() * 1000),
            "hypothesisId": hypothesis_id,
        }
...
class TrainFlowPolicyWorkspace:
    ...
    def run(self):
        ...
        _agent_debug_cuda_snapshot(
            "train.py:after_val_dataloader",
            "H5",
            {"stage": "after_val_dataloader", "model_numel": _nparam},
        )
        # #endregion
        ...
        # #region agent log
        _agent_debug_cuda_snapshot(
            "train.py:after_wandb_init",
            "H4",
            {"stage": "after_wandb_init"},
        )
        # #endregion
        ...
        # #region agent log
        _nparam2 = sum(p.numel() for p in self.model.parameters())
        _agent_debug_cuda_snapshot(
            "train.py:before_model_to",
            "H2",
            {
                "stage": "before_model_to",
                "model_numel": _nparam2,
        ...
        # #region agent log
        _agent_debug_cuda_snapshot(
            "train.py:after_env_runner",
            "H1",
            {"stage": "after_env_runner_after_model_to", "runId": "post-fix"},
        )
        # #endregion
